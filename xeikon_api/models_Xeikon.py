...
class DVL(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint("unit", "color", name="DVL_unit_color"),)
    unit = Column(String(10), ForeignKey('xeikon.unit'), nullable=False)
    color = Column(String(15), nullable=False)
    current = Column(Integer, nullable=False)
    replaced_total = Column(Integer, nullable=False)
    replace_dates = relationship("DVL_Repl", backref='xeikon_DVL')
    def __repr__(self):
...
class Fuser(Base):
    ...
    id = Column(Integer, primary_key=True)
    unit = Column(String(10), ForeignKey('xeikon.unit'), nullable=False, unique=True)
    current = Column(Integer, nullable=False)
    replaced_total = Column(Integer, nullable=False)
    def __repr__(self):
